...
### 1. GitOps & Observability Dashboards

![Argo CD Dashboard](docs/assets/screenshots/images/ARGO screenshot.png)
*Argo CD Dashboard: Showing synced and healthy status for multi-region ApplicationSets and progressive canary rollouts.*

![Hubble UI Network Topology](docs/assets/screenshots/images/Hubble_UI.png)
*Hubble UI / Cilium Service Map: Live eBPF network flow diagram rendering real-time gRPC/HTTP traffic between Go API Gateway, Rust Data Store, and Python Telemetry without sidecar overhead.*

![Backstage Software Catalog](docs/assets/screenshots/images/backstage catalog_without_data.png)
*Backstage Software Catalog: Developer portal interface demonstrating platform self-service capabilities (minimal deployment without configured catalog entities).*

### 2. CI/CD & Security Workflow Runs (GitHub Actions UI)

![CI Pipeline Matrix](docs/assets/screenshots/images/CI pipeline.png)
*Parallel CI Test Grid: Green build checks showing unit, integration, Pact contract, and Playwright E2E test runs.*

![Security Scan Results](docs/assets/screenshots/images/security scan results github actions.png)
*Supply Chain Security Results: Security summary tab showing dependency analysis and vulnerability scanning status.*

![FinOps PR Comment](docs/assets/screenshots/images/FinopsPR.png)
*FinOps PR Comment: A live GitHub Pull Request comment generated by the mock Infracost / Kubecost engine showing pre-merge cost diff estimations.*

![Cosign Image Verification](docs/assets/screenshots/images/cosign-verification.png)
*Cosign Image Signature Verification: Build log showing container image signing and verification status for supply chain security.*

### 3. Progressive Delivery & Resilience Demos

*Argo Rollouts Canary Analysis: Terminal or UI screenshot during an automated rollback triggered by a simulated 5xx error threshold in Prometheus.*

*Chaos Mesh Experiment Execution: Terminal or dashboard view showing pod-kill or network latency injection and auto-remediation in action.*
