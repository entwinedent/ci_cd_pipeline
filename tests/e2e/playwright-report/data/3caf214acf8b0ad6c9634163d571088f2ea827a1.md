...
- Name: api-tests.spec.ts >> Telemetry Log Ingestion >> POST /api/v1/logs should ingest logs successfully
- Location: api-tests.spec.ts:70:7
...
Call log:
  - → POST http://localhost:8000/api/v1/logs
...
> 81  |     const response = await request.post(`${TELEMETRY_URL}/api/v1/logs`, {
  ...
  147 | 
  148 |     // Step 4: Send metric about the operation
  149 |     const metricResponse = await request.post(`${TELEMETRY_URL}/api/v1/metrics`, {
  150 |       data: {
  151 |         service: 'api-gateway',
  152 |         metric_name: 'data_operations',
  153 |         value: 1,
  154 |         timestamp: new Date().toISOString(),
  155 |       },
  156 |     });
  157 |     expect(metricResponse.status()).toBe(200);
  158 | 
  159 |     // Cleanup
  160 |     await request.delete(`${API_GATEWAY_URL}/api/v1/data/${chainKey}`);
  161 |   });
  162 | });
  163 | 
```